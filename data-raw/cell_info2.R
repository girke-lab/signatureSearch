...
head(cellInfo2017)
#### double check if there is a slight name change between versions (e.g. HEK293 vs HEK293T cells)
diffs <- setdiff(cellInfo2020$cell_iname, cellInfo2017$base_cell_id) # base cell ID is more universal 
for(i in 1:length(diffs)){
    if(nrow(cellInfo2020[grepl(diffs[i], cellInfo2020$cell_iname, ignore.case = TRUE),]) > 1){
        print(c(cellInfo2020[grepl(diffs[i], cellInfo2020$cell_iname, ignore.case = TRUE),]$cell_iname, diffs[i]))}}
#### create new "cell_info" table ####
setnames(cellInfo2020, "cell_iname", "cell_id")
cell_info2020 <- merge(cellInfo2020, cellInfo2017[,c("cell_id", "primary_site")], by = "cell_id", all.x = TRUE)
dim(cell_info2020)
head(cell_info2020)
cell_info2 <- cell_info2020
