...
BiocManager::install("yduan004/signatureSearch")  # Installs from github
```
...
### Batch queries on a single machine
Users could easily run GESS analysis with multiple queries in batch
with `lapply` function on their local computer. 
For example, the following code shows how to run batch queries
for `LINCS` method searching against the sample database. If users want to 
search against the LINCS database, simply set `refdb` as `lincs`.
...
# "23645" etc. represents gene entrez ids
...
gess_list <- lapply(seq_along(batch_queries), function(i){
    ...
    # create a 'batch_res' directory under users current working environment and
    # write the GESS result tables to this dir. Users could change it to where they want
...
### Parallelization on a single machine with multiple cores
The batch queries could be paralleled on a single machine with multiple cores
as follows
...
### Parallelization on cluster via scheduler

Alternatively, the computation can be greatly accelerated by processing many queries 
in parallel using several compute nodes of a cluster, where a scheduling/queuing
system is used for load balancing.

`ncpus` defines the number of CPU cores on the compute nodes, the number of 
independent parallel cluster processes is defined under the `Njobs` argument. 
The following example will run 2 processes in parallel using for each 2 CPU cores. 
If the resources available on a cluster allow running all 2 processes at the same 
time then the shown sample submission will utilize in total 4 CPU cores. 
Note, this can be used with most queueing systems as it is based on utilities 
from the _`batchtools`_ package which supports the use of template files 
(_`*.tmpl`_) for defining the run parameters of different schedulers. To run the 
following code, one needs to have both a conf file 
(see _`.batchtools.conf.R`_ samples [here](https://mllg.github.io/batchtools/)) 
and a template file (see _`*.tmpl`_ samples 
...
# copy conf and template files for Slurm schedular to current working directory
...
ids <- batchMap(fun=f_bt, Njobs, more.args = list(
          batch_queries=batch_queries, refdb=refdb))
            ...
            partition="intel", walltime=120, ntasks=1, ncpus=2, memory=10240))
getStatus()
