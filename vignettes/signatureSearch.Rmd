...
author: "Authors: Yuzhu Duan, Dan S Evans, Nicholas J. Schork, Steven R. 
        Cummings and Thomas Girke"
...
```{r gess_cmap, eval=TRUE, warning=FALSE}
...
corresponding differential expression scores of the GESs in the reference
...
call are genome-wide signatures, here z-scores. The actual gene sets required 
...
fisher2 <- gess_fisher(qSig=qsig_fisher2, higher=1, lower=-1)
result(fisher2)
```
...
interest. For comparing the performance of 
...
correlation- and set-based methods can be more comparable because both are 
provided with equivalent information content. 
...
defining the `workers` parameter, the default is 4. It means that when submitting
...
cluster with a workload management (queueing) system (_e.g._ such as Slurm or
Torque). 
...
queueing system available on a cluster. The following example uses the sample `conf` and
`template` files for the Slurm scheduler. 
...
done <- submitJobs(ids, reg=reg, resources=list(
...
similarity to a query signature of interest. Interpreting these search results
...
test sets used for TSEA. The relevance of these target duplications is
explained above. The specialized enrichment algorithms include *Duplication
...
<font size="-2"> Figure 4: Ranking comparison of top GO terms across FEA
...
<font size="-2"> Figure 5: Ranking comparison among top KEGG pathways across FEA
...
<font size="-2"> Figure 6: Drug-target network for *NAD-dependent histone
...
<font size="-2"> Figure 7: Drug-target network for *NF-kappaB binding*. The 
details of the plot are given in the legend of Figure 6.</font>
...
<font size="-2"> Figure 8: Drug-target network for *Alcoholism*. The details of
the plot are given in the legend of Figure 6.</font>
...
<font size="-2"> Figure 9: Drug-target network for *Longevity regulating
pathway*. The details of the plot are given in the legend of Figure 6.</font>

# Run Workflow
The `runWF` function could run the entire GESS/FEA workflow automatically
...
(MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb/index.jsp) contains
well-annotated gene sets representing the universe of the biological processes
in human [@Liberzon2011-ta; @Liberzon2015-is; @Culhane2012-aa]. MSigDB Version 7.1 contains 25,724 gene sets 
that are divided into 8 collections. 

GSKB (Gene Set Knowledgebase) is a gene set database for pathway analysis in 
mouse [@Lai2016-gh]. It includes more than 40,000 pathways and gene sets 
compiled from 40 sources, such as Gene Ontology, KEGG, GeneSetDB from mouse. 

_signatureSearch_ provides utilities to import gene sets in `gmt` format from 
MSigDB, GSKB and related gene set-based resources. The imported gene sets can be used 
either as set-based queries or reference databases, or both. The gene set queries 
are compatible with the the `CMAP`, `LINCS` or `Fisher` GESS methods. In addition,
the gene set reference databases can be used with the `Fisher` and `gCMAP` 
methods. The following examples illustrates how to import and search `gmt` files 
in _signatureSearch_ using sample data from MSigDB and GSKB. First, examples are 
provided how to use the gene sets from both resources as queries (here Entrez IDs from human)
to search the CMAP2 and LINCS databases. Next, several examples are provided where
the MSigDB and GSKB collections serve as reference databases that are searched compatible
GESS methods. 

### Gene Sets as Queries 
The gmt files were downloaded from the MSigDB at
[here](https://www.gsea-msigdb.org/gsea/downloads.jsp#msigdb)
and imported into R session with `read_gmt` function. The selected gene set 
could be used as up-regulated gene set input for `CMAP`, `LINCS` or `Fisher` 
methods if the gene set were unlabeled. Since 
```{r gmt_query, eval=FALSE}
msig <- read_gmt("msigdb.v7.1.entrez.gmt") # path to the downloaded msigdb gmt file
db_path <- system.file("extdata", "sample_db.h5", package = "signatureSearch")
```

<p style="color:green">[ThG: comment please include in readLines call the setting
'warn = FALSE' of the read_gmt function. This way there is no irritating warning
if gmt has only one entry.]</p>

Gene set that do not have up down labels
...
cmap_res <- gess_cmap(cmap_qsig)
# LINCS method
lincs_qsig <- qSig(query=list(upset=gene_set), gess_method="LINCS", refdb=db_path)
lincs_res <- gess_lincs(lincs_qsig)
# Fisher methods
fisher_qsig <- qSig(query=list(upset=gene_set), gess_method="Fisher", refdb=db_path)
fisher_res <- gess_fisher(fisher_qsig, higher=1, lower=-1)
```

Gene set that have both up and down gene labels
...
#### MSigDB
The gene sets stored in the gmt file could also be used as the reference database
for `gCMAP` and `Fisher` methods. In this case, all gene sets are labeled as
up-regulated. The `gmt2h5` function first convert the gene sets stored in the
gmt file to a 01 matrix where rows are all combined genes from all gene sets
and columns are names of the gene sets, 1's and 0's indicating whether genes 
are in the specific gene set. The 01 matrix was then written to the HDF5 
file. To avoid overflowing the memory, this function supports reading and writing
the gene sets in batches by defining the `by_nset` parameter. The generated HDF5
file was used as the GS-DB for `gCMAP` and `Fisher` methods. Please note, when
using the HDF5 file converted from the gmt file as `refdb`, the `higher` and
`lower` parameters of the `gess_gcmap` and `gess_fisher` functions need to be
set as `1` and `NULL`, respectively.
```{r gn_ref, eval=FALSE}
gmt2h5(gmtfile="./msigdb.v7.1.entrez.gmt", dest_h5="./msigdb.h5", by_nset=5000, 
       overwrite=TRUE)

# gCMAP method
query_mat <- getSig(cmp="vorinostat", cell="SKB", refdb=db_path)
gcmap_qsig2 <- qSig(query=query_mat, gess_method="gCMAP", refdb="./msigdb.h5")
gcmap_res2 <- gess_gcmap(gcmap_qsig2, higher=1)
...
fisher_qsig2 <- qSig(query=list(upset=gene_set), gess_method="Fisher", 
                    refdb="./msigdb.h5")
fisher_res2 <- gess_fisher(fisher_qsig2, higher=1)
```

<p style="color:green">[ThG: I am not able to run any of the gess_ searches
when the gene sets are used as db on a laptop with 8GB RAM. Usually this is 
plenty. Reducing the chunk size also doesn't help. The searches also seem to be
very slow in this case. Not sure are you simply bypassing the HDF5 encoding of the 
ref database? If so why? For the review this could be a deal breaker if one makes
the impression that the performance on these relatively small databases is so 
poor.]</p>


#### GSKB 
The gmt file of the GSKB database for mouse is downloaded from
[here](http://ge-lab.org/#/data). The following code makes up a query signature
with Mouse Entrez ids for the `gCMAP` method and uses the mouse gene sets as
reference database. The gene sets could also be used as both query and reference 
database for `Fisher` methods. Please note, the mouse gene set could also be 
used as query set for `CMAP` and `LINCS` methods if the reference database 
contains mouse genome-wide gene expression profiles from various treatments.


<p style="color:green">[ThG: the read_gmt call issues warnings which should not be
the case. Also, why couldn't the second line for removing empty entries be included in
read_gmt function? This way the user would not need to deal with complex 
expressions.]</p> 

```{r gn_ref_mouse, eval=FALSE}
gmt2h5(gmtfile="./mGSKB_Entrez.gmt", dest_h5="./mGSKB.h5", by_nset=5000, 
       overwrite=TRUE)

# gCMAP method
## make up the query matrix
gskb <- read_gmt("mGSKB_Entrez.gmt")
...
gcmap_qsig3 <- qSig(query=mquery, gess_method="gCMAP", 
                    refdb="./mGSKB.h5")
gcmap_res3 <- gess_gcmap(gcmap_qsig3, higher=1)
...
fisher_qsig3 <- qSig(query=list(upset=gene_set), gess_method="Fisher", 
                    refdb="./mGSKB.h5")
fisher_res3 <- gess_fisher(fisher_qsig3, higher=1)
```

<p style="color:green">[ThG: same problem here. The R session simply crashes.]</p>
