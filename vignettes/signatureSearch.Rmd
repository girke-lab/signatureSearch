...
# Gene Sets in gmt File
The Molecular signatures database (MSigDB) contains well-annotated gene sets 
representing the universe of the biological processes in human [@Liberzon2011-ta]. 
The MSigDB is available at its 
[website](https://www.gsea-msigdb.org/gsea/msigdb/index.jsp). 
At the time of writing this vignette, the MSigDB version is 7.1. It contains 
25,724 gene sets divided into 8 collections. 
...
compiled from 40 sources such as Gene Ontology, KEGG, GeneSetDB etc in mouse. 

This package also provides utilities to read in gene sets stored in `gmt` file 
and uses the imported gene sets as either query set or reference database. The
gene set query is applicable to the `CMAP`, `LINCS` or `Fisher` GESS methods. 
The gene set reference database is applicable to the `Fisher` and `gCMAP` 
methods. The following shows examples of using gene sets stored in `gmt` file
to the different GESS methods. Since the CMAP/LINCS database contains Entrez IDs
in human, the following 2 subsections uses gene sets in MSigDB as illustration.
The third subsection `Gene set in mouse` illustrates how to use the mouse 
gene set as query or reference database for GESS methods
...
## Gene set as reference database
