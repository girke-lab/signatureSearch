...
<font color="red">[ThG-Comment: how has this discussion been addressed?]</font>
...
The `comp_fea_res` function ranks the functional categories across the
different FEA methods by using the mean rank of each functional category across
the 5 FEA methods, <font color="green">the lower of the mean rank, the topper
of the functional category</font>. <font color="blue">[ThG-Comment: I don't
understand the green part of this sentence. Please revise.]</font> Since the
functional categories are not always present in all enrichment results, the mean 
rank of a functional category is corrected by an adjustment factor that is the
the total number of occurences of a functional category divided by the number of 
result methods. For instance, if a functional category is only present in the
result of one method, its mean rank will be downgraded accordingly. 
The following plots use the `pvalue` column in the result tables for this ranking 
approach. Alternative columns can be chosen under the `rank_stat` argument. After 
re-ranking only the top ranking functional categories are shown, here 20. Their 
number can be changed under the `Nshow` argument. 

<font color="blue">[ThG-Comment: you missed to add a legend/caption under the plots.]</font>
