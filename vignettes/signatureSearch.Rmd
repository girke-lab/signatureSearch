...
<center>
![](images/gess_fea_wf.png){width=18cm}
</center>
...
If the signature database contains drug treatments in different cell types, including normal and tumor, the GESS result can be visualized by summarizing rankings/scores of selected drugs across difference cells. The sample databased used in this vignette only contains treatments in MCF7 cell, so there is no need to summarize the result. I used the same `vorinostat` query signature to search the full LINCS database by LINCS method, Figure 3 shows the visualization of summary GESS result.

<center>
![](images/vis_gess_res.png)
</center>

Figure 3: Summary of NCS scores across cell types of selected drugs in vorinostat GESS result.
...
## Compare consistency of enriched terms from 5 FEA methods
```{r cmp_enrich, eval=TRUE, fig.width=10.5, fig.height=5.5}
comp_fea_res(dup_hyperG_res, mgsea_res, mabs_res=NULL, hyperG_res, gsea_res, Ntop=20, type="GO")
```

```{r cmp_enrich_k, eval=TRUE, fig.width=8}
comp_fea_res(dup_hyperG_k_res, mgsea_k_res, mabs_res=NULL, hyperG_k_res, gsea_k_res, Ntop=20, type="KEGG")
```
