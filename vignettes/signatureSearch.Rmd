...
The `comp_fea_res` function re-ranks the functional categories from different
FEA methods by using the mean rank of each functional category
across the 5 FEA methods, the lower of the mean rank, the topper of the 
functional category. Since the functional categories have different number of 
support methods, the mean rank of a functional category is adjusted by 
multiplying the total number of methods and divided by the number of support 
methods. In this way, if a functional category is supported by only one method,
the mean rank will be increased 5 times for penalty. 

The following plots rank the functional categories of each method by `pvalue`
and show the p-values of the top 20 re-ranking functional categories. 
