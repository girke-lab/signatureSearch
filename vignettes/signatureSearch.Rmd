...
qsig_cmap <- qSig(qsig = list(upset=upset, downset=downset), 
                  gess_method="CMAP", refdb=sample_db, refdb_name="sample")
cmap <- gess_cmap(qSig=qsig_cmap, chunk_size=5000) # type "yes" when prompted
cmap
result(cmap)
```
...
qsig_lincs <- qSig(qsig = list(upset=upset, downset=downset), 
                   gess_method="LINCS", refdb=sample_db, refdb_name="sample")
...
qsig_gcmap <- qSig(qsig = query_mat, gess_method = "gCMAP", 
                   refdb = sample_db, refdb_name="sample")
...
qsig_fisher <- qSig(qsig = query_mat, gess_method = "Fisher", 
                    refdb = sample_db, refdb_name="sample")
fisher <- gess_fisher(qSig=qsig_fisher, higher=1, lower=-1)
result(fisher)
```

## Searching with correlation based method for GESS
...
methods (\emph{CMAP}, \emph{LINCS} and \emph{Fisher}).
...
```{r gess_sp, eval=TRUE}
qsig_sp <- qSig(qsig = query_mat, gess_method = "Cor", 
                refdb = sample_db, refdb_name="sample")
...
qsig_spsub <- qSig(qsig = query_mat_sub, gess_method = "Cor", 
                   refdb = sample_db, refdb_name="sample")
