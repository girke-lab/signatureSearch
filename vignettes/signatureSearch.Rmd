...
drug sets are converted into target gene or protein sets to perform Target Set
...
drugs directly to perform Drug Set Enrichment Analysis (DSEA). Although TSEA and DSEA
are related, their enrichment results can be distinct. This is mainly due to
duplications that are frequently present in test sets of TSEA, while the drugs
in the test sets of DSEA are usually unique. Simply uniquifying the proteins in the 
test sets of the TSEA approach would erase the most important enrichment information 
leading to meaningless results. Thus, a weighting method is used to preserve this
duplication information in case of TSEA as much as possilbe. To illustrate the 
importance of this: the top ranking drugs of a GESS result may predominantly 
bind to a single or a few target protein(s) resulting in a protein test set 
with many duplications, which is a highly desirable outcome. Thus, maintaining 
the duplications in the test set is essential for detecting these types of target 
enrichments. 
