...
author: "Authors: Yuzhu Duan, Dan S Evans, Nicholas J. Schork, Steven R. Cummings and Thomas Girke"
...
<font color="green">[YD-response: I used the matrix of gene scores in unsorted
way, not the sorted list since in this way, the format is matrix, which is unit,
easy to understand and could save the storage space.]</font>

<font color="blue" size="-2">[ThG-Comment: still the illustration of the method 
would be easier to understand if it showed here sorted values which is important 
for the algorithm. Keep in mind the illustration is more for explaining the algorithm
than the details of the implementation.]</font>
...
(here SKB cells). This would result in a not very informative plot. To illustrate 
the full potential of the `gess_res_vis` function, the following code section
applies to a search where the `vorinostat` signature was used to query with the
`gess_lincs` method the full LINCS database. Subsequently, the search result is
processed by the `gess_res_vis` function to generate the plot shown in
Figure 3. 
...
<font color="green">[YD-response: I think if it is necessary, we could add
the ChEMBL to the drug-target mapping resources as updates after 
submitting/releasing the package. For a query drug, I use the combined target
set from DrugBank, CLUE and STITCH databases as its targets. I think the 
combined target set is more complete than individual ones.]
</font>

<font color="blue">[ThG-Comment: after the submission of the signatureSearch
package let's design a function in the drugTargetInteraction package that
accomplishes this.]</font>
...
information of the annotation system, and the ontology type of the GO annotation
...
<font color="green">[YD-response: The combined targets from DrugBank, CLUE and
STITCH databases are used because if choosing specific annotation resource, such
as DrugBank, the drugs included is limited, many drugs will not get targets. 
The combined one could get targets of as many drugs as possible and the target 
set is more complete. So, I don't recommend allowing users to choose specific 
resource, in this way, the result will also depend on the chosen resource, which 
increases the complex and not complete.]</font>

<font color="blue">[ThG-Comment: allowing users to choose here their own
drug-target annotations seems essential to me. Your concerns could easily be
addressed by a default argument using your current drug-target
annotations. Most users would want this option. It would also be a more generic
and extendable design of the software if it were supported. The corresponding
argument description in the help file could point out limitations of what to
expect when too sparse drug-target annotations are used instead.]</font>
...
<font color="green">[ThG-Comment: I am not able understand the text in the following 
paragraph. I can guess what was done based on the figure but the text is not 
understandable.]</font>

The adjusted p-values from *mGSEA* and *GSEA* enrichment methods are much higher
than other methods, it is unfair and not applicable to select significantly 
enriched GO terms or KEGG pathways by setting cutoff (e.g. 0.05) on adjusted
p-values since in this way, no term will be selected from *mGSEA* method and 
too many terms will be selected from hypergeometric test. So here, the 
collection of top 20 terms from each FEA method is seemed as significant. 
To find the most confident and significant ones that are supported by most 
methods, the functional categories is re-ranked by putting the 
terms supported by more methods to topper. For terms supported by the same 
number of methods, the lower of the mean of ranks of the terms across 
enrichment methods, the topper of the terms. The `comp_fea_res` function 
shows the adjusted p-values and supported enrichment methods of the top 20 
re-ranked GO terms or KEGG pathways. It shows whether the terms are in the top
20 of the enrichment results from 5 methods and their corresponding adjusted 
p-values.

<font color="blue">[ThG-Comment: I am still confused after reading this
section several times. I am wondering whether you are using here a voting-based
reranking mechanism that considers the top 20 categories of each FEA method. If
so then I suggest to state this at the beginning of the above text and drop the
discussion of the p-value differences. To me this doesn't belong here. For the
reranking have you considered using the mean rank of each functional category
across the 5 FEA methods which would less often result in ambiguities for
reranking than a voting mechanism. Also why doesn't the function have an
argument to choose the actual statisics used for the ranking, e.g. raw p-value
or adjusted p-value. By choosing for the toy example the raw p-value, the plot
would not be so desert-like with respect to the colors used for the dots. Right
now they are all the same within each method as well as across most methods. To
the reader this would mean that the ranking shown in the plots is rather
arbitrary which you may want to avoid. We should discuss both the plotting
function and the write-up of this section. 

Another concern I have with the design of this function, is the choice of the
many arguments for the input result tables. To me there should be just one
argurment where the result objects are provided in a list-like structure. This
way the user can provide any number of input result tables.]</font>
...
<font color="blue">[ThG-Comment: I don't see that you have addressed the
text overplotting problem along the x-axis. To resolve
this you want to plot the labels vertically by turning them 90 degrees or 
use a plot legend on the side instead. In ggplot2 this is so super easy. 
In addition, the plots should include a short Figure legend. The titles
included on the top of the plots themselves could be dropped or at least 
shortened.]</font>
...
Functional modules of the above GESS and TSA results can be rendered as
...
identifier of a chosen functional category are passed on to the `drug` and
...
High-Performance Computing (HPC) resources used for optimizing and applying the 
code of this project were funded by NIH and NSF grants 1S10OD016290-01A1 and MRI-1429826,
respectively. 
