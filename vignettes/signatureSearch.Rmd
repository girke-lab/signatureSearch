---
title: _signatureSearch_ - Environment for Gene Expression Searching Combined with Functional Enrichment Analysis
...
<!--
This package implements algorithms and data structures for searching gene
expression signature (GESs) databases with query GESs to identify cellular
states sharing similar expression responses (connections). This way one can
discover perturbations, such as knockouts, diseases or drugs inducing
expression phenotypes similar to a sample of interest. Establishing these GES
connections can lead to the indentification of novel functional and regulatory
relationships of molecular and cellular processes, _e.g._ drugs-target
interactions.
-->
...
</font> <font color="blue" size="-2">[ThG-Comment: (1) The headers in the top
of the figure are inconsistently formatted. In one case you spell out the
acronym in the other you don't. (2) The GESS data type representation lack
rank-based GESs used by CMAP/gCMAP. The term profile refers to
normalized/unnormalized intensities/count data. (3) Under the GESS panel your
illustration would be more intuitive to understand if you provided informative
sub-titles such as "Query & DB Types" and "Search Methods"]</font>
...
the LINCS search method will be performed on ranked gene expression profiles
...
by the affiliated _signatureSearchData_ package. The GESS query parameters are defined
in a *qSig* search object where users can choose among five GESS methods (CMAP, LINCS,
gCMAP, Fisher, and Spearman correlation). Signature
...
methods. The enrichment results are stored as *feaResult* object that can be used
for drug target-networks analysis and visualization.</font>
...
As Bioconductor package `signatureSearch` can be installed with the `BiocManager::install()`
function.
...
variables. The referenced databases store the following information: (1)
...
time of ?? and ??, respectively. <font color="blue">[ThG-Comment: fill in
values under '??', and or improve as necessary.]</font> If neccessary one can
...
<font color="blue">[ThG-Comment: it is confusing that the objects from this
section are not used anywhere in the sample code of this vignette. I am also a
bit lost why we went through the effort of developing `signatureSearchData` but
don't use it here?]</font>
...
derived from a genome-wide profiling technology that my differ from the one
...
least comparable technologies. In other words, set-based methods are more technology 
agnostic than correlation-based methods, but may not provide the best precision
performance.


## Test Query and Database

To minimize the run time of the following test code, a small toy database has been
assembled from the LINCS database containing a total of 100 GESs from human SKB
(muscle) cells. Of the 100 GESs in this toy database, 95 were random sampled
and 5 were cherry-picked. The latter five are GESs from HDAC inhibitor
treatments including the known drugs: vorinostat, rhamnetin, trichostatin A,
pyroxamide, and HC toxin. The query signature used in the below sample code is
...
(here  `sample_db`). In addition, a test query set of up/down DEGs (here `upset` and 
...
most up- and down-regulated genes from a genome-wide expression experiment, while
the reference database is composed of rank transformed expression profiles
(_e.g._ ranks of LFC or Z-scores). The actual GESS algorithm is based on a vectorized
rank difference calculation. The resulting Connectivity Score expresses to 
what degree the query up/down gene sets are enriched on the top and bottom of 
the databases entries, respectively. The search results are a list of perturbagens
such as drugs that induce similar or opposing GESs as the query. Similar GESs 
suggest similar physiological effects of the corresponding perturbagens. As discussed 
in the introduction, these GES associations can be useful to uncover novel MOAs of drugs or 
treatments for diseases. Although several variants of the CMAP algorithm are 
available in other software packages including Bioconductor, the implementation 
provided by `signatureSearch` follows the original description of the authors as 
closely as possible and allows to reproduce the search results relatively well that are 
obtainable from the corresponding CMAP2 web service provided by the Broad Institute.

Generate `qSig` object for `gess_cmap` method and search against the sample
reference database <font color="blue">[ThG-Comment: please describe here what
...
result(cmap)
```
<font color="blue">[ThG-Comment: here and in the following you need to briefly
...
<font color="blue">[ThG-Comment: this is as far as I have edited this vignette
