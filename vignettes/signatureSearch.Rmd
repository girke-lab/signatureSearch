...
The `query` could also be a list of up and down regulated gene id label sets
as with the *CMAP* or *LINCS* method. In this case, genes in up and down sets 
are combined into a single gene set for Fisher's exact test with reference 
gene sets, which means genes in the query set are unsigned. The `refdb` is 
still genome-wide z-scores, the gene sets in reference database is obtained by 
setting the `higher` and  `lower` cutoffs to 1 and -1, respectively.

```{r gess_fisher_set, eval=TRUE, message=FALSE}
...
If the reference database contains perturbations (_e.g._ drugs) of different 
cell types, including normal and tumor cell classifications, the GESS result 
can be visualized by summarizing rankings/scores of selected drugs across 
difference cells. The sample databases used in this vignette only contains 
treatments in SKB cell, so there is no need to summarize the result. 
To show the visualization of the complete search result, the following code uses
the same `vorinostat` query signature to search against the full LINCS database 
via `gess_lincs` method, then uses `gess_res_vis` function to generate the
plot (Figure 3). 
...
database with *LINCS* method. The other drugs are 10 HDAC inhibitors not 
selected in the toy database. For each drug, the colored dots represent NCS scores in
normal cell types. The colored triangles represent NCS scores in tumor cell
types. The black dot and triangle represent summarized NCS scores across normal
and tumor cell types, respectively. The NCS is summarized across cell types 
by obtaining a maximum quantile statistic. It compares the 67 and 33 quantiles 
of NCS in different cells and retains whichever is of higher absolute magnitude,
which is similar as `NCSct` score in the result table from `gess_lincs` 
function. </font>

<font color="green">[ThG-Comment: This section needs to include the code for
...
control FDR; `itemID`: IDs of items (genes for TSEA, drugs for DSEA) overlapping 
...
The combined one could get targets of as many drugs as possible and the target set is more complete. So, I don't recommend allowing users to choose specific resource, in this way, the result will also depend on the chosen resource, which increases the complex and not complete.]</font>
...
The following performs DSEA with the *GSEA* method using the drug test set
with scores. Instead of using only the drug labels in the test
...
values from LINCS. The arguments of the `dsea_GSEA` function are explained in 
its help file that can be opened from R with `?dsea_GSEA`.
...
The above GESS and TSEA results can be rendered as interactive drug-target
networks using the `dtnetplot` function from `signatureSearch`. For this, a
character vector of drug names along with an identifier of a chosen functional
category are passed on to the `drug` and `set` arguments, respectively. The
resulting plot depicts the corresponding drug-target interaction network.  Its
interactive features allow the user to zoom in and out of the network, and to 
select network components in the drop-down menu located in the upper left corner 
of the plot.
