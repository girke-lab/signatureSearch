...
the drugs in the test sets of DSEA are usually unique. Another reason are
...
annotations provide UniProt identifiers for the target proteins. These can be
...
`signatureSearch` along with its helper package `signatureSearchData` provide
many convenience utilities along with drug-target lookup resources for
automating the mapping, often reaching from _drug sets_ to _target sets_ to
_functional categories_.

<font color="blue">[ThG-Comment: I am wondering whether we should limit the
drug-target mapping resource provide by the data package to the most reliable
and maintainable resource (e.g. DrugBank and/or ChEMBL). Especially, including
STICH seems redundant.  I also don't like their predicted drug-target
associations. If necessary the same could be obtained from ChEMBL.] </font>
