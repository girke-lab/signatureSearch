...
steps of a typical signature search workflow. For each GESS and FEA step, 
several alternative methods have been implemented in _signatureSearch_ to allow users
to choose the best possible workflow configuration for their research application. 
The individual search and enrichment methods are introduced in the corresponding 
...
call are genome-wide signatures, here z-scores. The actual gene sets requied for
the Fisher's exact test are obtained by setting `higher` and `lower` cutoffs
corresponding in this case to up and down regulated genes, repectively.
...
the query contains the same gene identifiers as used in the reference database. The
expected data structure of the query is a matrix with a single numeric column and the
...
such as a DEG set or a pathway of interest. For comparing the performance of 
...
such as the up/down DEGs used in a _LINCS_ GESS. This way correlation-based 
search results can be more comparable to a set-based method by providing to each 
method a more equal amount of information than this is the case for the 
correlation method with genome-wide GESs. 
...
since SP implies the method is specific to Spearman which isn't and shouldn't
be the case.]</font>

## Summary of Search Results

Four of the five cherry-picked HDAC inhibitors (vorinostat, trichostatin-a,
HC-toxin, pyroxamide) were ranked among the top 10 ranking drugs in the search
results of the  LINCS, Fisher and Spearman correlation methods. This
demonstrates a promising performance of these search methods for grouping drugs
by their MOA categories. In addition, the LINCS and Spearman methods were able
to rank another HDAC inhibitor, here APHA-compound-8, at the top of their
search results. This indicates a better sensitivity of these two methods
...
in the texts that the plot is based on such a search. Also, the text above and the
corresponding gess_res_vis help file are not clear enough to understand what
is plotted here, e.g. what is does each dot represent: a single NCS value, an 
average or something else? In the legend of the plot you may want to change the 
subtitle on right side "cell" to "cell_type" and the one below from "cell_type" to
"cell_class"]</font>
...
`ont`: One of `BP`, `MF`, `CC` or `ALL` if GO annotation system. If KEGG 
...
"nPerm" times and dividing the enrichment score by mean of the permutation $ES$
with the same sign.

`pvalue`: The nominal p-value of the $ES$ is calculated using permutation test. 
Specifically, the gene labels of the gene list $L$ were permuted and the $ES$ of 
...
deviation from zero. Can be interpreted as the core of a gene set that accounts 
