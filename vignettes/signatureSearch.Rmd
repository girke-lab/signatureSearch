...
cells is provided. The GEPs can be log2 fold change, z-scores *etc.* from 
...
object. The query signature and reference database were built into *qSig* 
...
stored in *gessResult* object and can be functionally annotated by TSEA 
and DSEA methods. The enrichment results are stored in *feaResult* object 
...
signature and reference database are built into *qSig* object and 5 GESS 
methods (CMAP, LINCS, gCMAP, Fisher, and Spearman) are supported for searching. 
The search results are stored in *gessResult* object and can be 
functionally annotated by TSEA (dup_hyperG, mGSEA, mabs) and DSEA (hyperG, GSEA)
methods. The enrichment results are stored in *feaResult* object and the 
