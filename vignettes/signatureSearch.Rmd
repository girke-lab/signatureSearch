...
regulated gene labels, the reference database should be rank transformed 
...
similarity to the query. For the _CMAP_ method, the similarity metrics are 
...
signature. It was scaled to values from 1 to -1 by dividing the positive scores
with the maximum positive score, and negative scores with the absolute value of
...
The generation of `qSig` object for _LINCS_ method is similar with _CMAP_. The 
`gess_method` argument is defined as "LINCS" and _LINCS_ method is applied on 
...
set. If ES of up and down sets are of different signs, WTCS is (ESup-ESdown)/2, 
otherwise, it is 0. WTCS ranges between -1 and 1. It will be positive for 
...
relative to query $q$. A tau of 90 indicates that only 10 percent of reference
perturbations showed stronger connectivity to the query.
...
are GESs with z-scores. The `gess_cor` function also support Pearson and Kendall 
