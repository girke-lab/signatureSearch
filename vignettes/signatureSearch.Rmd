...
<p style="color:red">[ThG: comment please include in readLines call the setting
...
gcmap_qsig2 <- qSig(query=query_mat, gess_method="gCMAP", 
                    refdb="msigdb.v7.1.entrez.gmt")
gcmap_res2 <- gess_gcmap(gcmap_qsig2)
...
fisher_qsig2 <- qSig(query=list(upset=gene_set), gess_method="Fisher", 
                    refdb="msigdb.v7.1.entrez.gmt")
fisher_res2 <- gess_fisher(fisher_qsig2)
```

<p style="color:red">[ThG: I am not able to run any of the gess_ searches
...
contains mouse genomewide gene expression profiles from various treatments.


<p style="color:red">[ThG: the read_gmt call issues warnings which should not be
...
gskb <- gskb[sapply(gskb, length)>0 & ! is.na(names(gskb))]
...
gcmap_qsig3 <- qSig(query=mquery, gess_method="gCMAP", 
                    refdb="mGSKB_Entrez.gmt")
gcmap_res3 <- gess_gcmap(gcmap_qsig3)
...
fisher_qsig3 <- qSig(query=list(upset=gene_set), gess_method="Fisher", 
                    refdb="mGSKB_Entrez.gmt")
fisher_res3 <- gess_fisher(fisher_qsig3)
```

<p style="color:red">[ThG: same problem here. The R session simply crashes.]</p>
