...
comp_fea_res <- function(dup_hyperG_res=NULL, mgsea_res=NULL, mabs_res=NULL, hyperG_res=NULL, gsea_res=NULL, Ntop=20, type="GO"){
  ...
  if(! is.null(dup_hyperG_res)){
    ...
    dup_df <- data.frame(dup_tb[1:min(nrow(dup_tb), Ntop),c("Description","p.adjust")], method="dup_hyperG", rank=1:min(nrow(dup_tb), Ntop))
    df <- rbind(df, dup_df)
  }
  if(! is.null(mgsea_res)){
    mgsea_tb <- result(mgsea_res)
    mgsea_df <- data.frame(mgsea_tb[1:min(nrow(mgsea_tb), Ntop),c("Description","p.adjust")], method="mGSEA", rank=1:min(nrow(mgsea_tb), Ntop))
    df <- rbind(df, mgsea_df)
  }
  if(! is.null(mabs_res)){
    mabs_tb <- result(mabs_res)
    mabs_df <- data.frame(mabs_tb[1:min(nrow(mabs_tb), Ntop),c("Description","p.adjust")], method="mabs", rank=1:min(nrow(mabs_tb), Ntop))
    df <- rbind(df, mabs_df)
  }
  if(! is.null(hyperG_res)){
    hpg_tb <- result(hyperG_res)
    hpg_df <- data.frame(hpg_tb[1:min(nrow(hpg_tb), Ntop),c("Description","p.adjust")], method="hyperG", rank=1:min(nrow(hpg_tb), Ntop))
    df <- rbind(df, hpg_df)
  }
  if(! is.null(gsea_res)){
    gsea_tb <- result(gsea_res)
    gsea_df <- data.frame(gsea_tb[1:min(nrow(gsea_tb), Ntop),c("Description","p.adjust")], method="GSEA", rank=1:min(nrow(gsea_tb), Ntop))
    df <- rbind(df, gsea_df)
  }
  # rank descriptions
  ## first rank according to frequence
  freq <- as.factor(sort(table(df$Description), decreasing = TRUE)[1:Ntop])
