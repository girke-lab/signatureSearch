...
fgsea2 <- function(pathways, stats, nperm,
    ...
    message("Using fgsea2 function to run GSEA, which is adapted to accepting 'stats' with large portion of zeros.")
    if (is.null(BPPARAM)) {
        if (nproc != 0) {
    ...
    counts <- bplapply(seq_along(permPerProc), function(i) {
        ...
        if (m == 1) {
            ...
            #   iterations = nperm1,
            #   seed = seeds[i]), "~/insync/project/hl/fgsea_test/aux_param.rds")
