...
moa_mrk <- function(gess_tb, moa_list, cells){
  gess_tb %<>% mutate(rank=1:nrow(gess_tb))
  if(cells=="all"){
    moa_mrk <- sort(sapply(moa_list, function(x) mean(as.numeric(gess_tb$rank[gess_tb$pert %in% x]), na.rm=TRUE)))
    moa_mrk_df <- data.frame(moa=names(moa_mrk), cell="all", mean_rank=moa_mrk, row.names = NULL, stringsAsFactors = FALSE)
  }
    ...
    moa_mrk <- sort(sapply(moa_list, function(x) mean(as.numeric(gess_tb_sub$rank[gess_tb_sub$pert %in% x]), na.rm=TRUE)))
    moa_mrk_df <- data.frame(moa=names(moa_mrk), cells="normal", mean_rank=moa_mrk, row.names = NULL, stringsAsFactors = FALSE)
  }
  else if(cells=="tumor"){
    ...
    gess_tb_sub <- filter(gess_tb, cell %in% cell_sel)
    moa_mrk <- sort(sapply(moa_list, function(x) mean(as.numeric(gess_tb_sub$rank[gess_tb_sub$pert %in% x]), na.rm=TRUE)))
    moa_mrk_df <- data.frame(moa=names(moa_mrk), cells="tumor", mean_rank=moa_mrk, row.names = NULL, stringsAsFactors = FALSE)
  } else {
    cell_sel <- cells
    gess_tb_sub <- filter(gess_tb, cell %in% cell_sel)
    moa_mrk <- sort(sapply(moa_list, function(x) mean(as.numeric(gess_tb_sub$rank[gess_tb_sub$pert %in% x]), na.rm=TRUE)))
    moa_mrk_df <- data.frame(moa=names(moa_mrk), cells="group", mean_rank=moa_mrk, row.names = NULL, stringsAsFactors = FALSE)
  }
  moa_num <- data.frame(moa=names(moa_list), n_drug=sapply(moa_list, length), stringsAsFactors = FALSE)
