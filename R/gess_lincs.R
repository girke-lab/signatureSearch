...
.lincsScores <- function(esout, upset, downset, minTauRefSize, 
  ...
  if(!all(queryDB_refDB_match)) 
    warning(paste0("QueryDB and tauRefDB differ by ", 
...
lincsEnrich <- function(se, upset, downset, sortby="NCS", type=1, 
  ...
  if(output=="all") {
    resultDF <- .lincsScores(esout=ESout, upset=upset, downset=downset, 
        minTauRefSize=minTauRefSize, ES_NULL=ES_NULL)
  }
  if(!is.na(sortby)) {
...
#' implemented here. 
#' 
#' The LINCS method takes about 10 min on a single CPU core for querying 
#' with a single signature against ~100,000 signatures in the LINCS database.
#' 
#' @param qSig `qSig` object, The 'gess_method' slot should be 'LINCS'
#' @param ES_NULL path to the ES_NULL file. ES null distribution is generated 
...
#' @references For detailed description of the LINCS method, please refer to
#' Subramanian et al., 2017,
...
#' result(lincs)
#' @export
gess_lincs <- function(qSig, ES_NULL="Default", 
  ...
  res <- lincsEnrich(se=se, upset=upset, downset=downset, ES_NULL=ES_NULL, 
                     sortby=sortby, chunk_size=chunk_size)
  # add target column
